...
export default {
  ...
  provideLinter: () => {
    ...
    return {
      ...
      lintOnFly: false,
      lint: (activeEditor) => {
        const command = atom.config.get("linter-gcc.execPath");
        ...
        // }
        //
        // args.push(`-ferror-limit=${atom.config.get("linter-gcc.gccErrorLimit")}`);
        ...
        // }
        //
        atom.config.get("linter-gcc.gccIncludePaths").forEach((path) =>
          args.push(`-I${path}`)
        );
        //
        ...
        args.push(file);
        // atom.notifications.addError( "args", {  detail: args } );
