...
module.exports = {
  ...
  lint: function(editor, linted_file, real_file){
    ...
    if (atom.config.get("linter-gcc.gccRelintMessageSources") == true){
      for (var file in module.exports.messages){
        ...
        for (var msg in fileMessages){
          ...
          if (message.filePath === real_file && file !== real_file && !module.exports.last_linted_files.has(file)){
            otherFiles.add(file);
          }
        }
      }
    ...
    return helpers.exec(command.binary, command.args, {stream: "stderr"}).then(output => {
      msgs = require("./utility.js").parse(output, regex)
      msgs.forEach(function(entry){
        ...
        if (entry.severity ===`${atom.config.get("linter-gcc.gccWarningString")}`)
            entry.severity = "warning";
        else if (entry.severity ===`${atom.config.get("linter-gcc.gccErrorString")}`)
            entry.severity = "error";
        else if (entry.severity ===`${atom.config.get("linter-gcc.gccNoteString")}`)
            entry.severity = "info";
      })
