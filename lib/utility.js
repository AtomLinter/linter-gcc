module.exports = {
  ...
  parse: function(data,editor) {
    ...
    while (rawMatch !== null) {
      ...
      var gccMessage = require("./gccMessage.js").build(colStart, text);
      var colEnd = gccMessage.colEnd();

      // if (typeof editor !== 'undefined' && editor && editor.getPath() == file) {
      //   var fileText = editor.getBuffer();
      //   var lineText = fileText.lineForRow(lineStart);
      //   // colStart = lineText.search(/\S|$/);
      //   colEnd = lineText.length;
      // } else {
      //   var editors = atom.workspace.getTextEditors();
      //   for (let i = 0; i < editors.length; i++) {
      //     if (editors[i].getPath() === file ) {
      //       var fileEditor = editors[i];
      //       var fileText = fileEditor.getBuffer();
      //       var lineText = fileText.lineForRow(lineStart);
      //       // colStart = lineText.search(/\S|$/);
      //       colEnd = lineText.length;
      //     }
